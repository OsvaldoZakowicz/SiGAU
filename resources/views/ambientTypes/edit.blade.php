    ...
    <div class="w-full h-max flex flex-col p-1">
        ...
        <div class="bg-zinc-300">
            ...
            <div class="p-1 flex justify-center items-center bg-zinc-400">
                <h3 class="text-sm font-bold text-zinc-800 uppercase inline-block">tipos de ambiente: editar tipo</h3>
            </div>
        </div>
        {{-- formulario --}}
        <div class="my-2 mx-auto w-full border bg-white border-zinc-200">
            <form action="{{ route('ambienttypes.update', $tipoAmbiente->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="flex flex-col items-center justify-center">
                    ...
                    <div class="w-full mt-2 p-2">
                        <x-required-input-label for="name" :value="'nombre del tipo de ambiente'"
                            title="ingrese el nombre del tipo de ambiente" />
                        @error('name')
                            <span class="text-xs text-red-600">{{ $message }}</span>
                        @enderror
                        <input name="name" value="{{$tipoAmbiente->name}}" type="text" required
                    ...
                    <div class="w-full mt-2 p-2">
                        <x-required-input-label for="description" :value="'descripcion del tipo de ambiente'"
                            title="ingrese una descripcion sobre el tipo de ambiente" />
                        @error('description')
                            <span class="text-xs text-red-600">{{ $message }}</span>
                        @enderror
                        <input name="description" value="{{$tipoAmbiente->description}}" type="text" required
                            class="my-1 p-1 w-full rounded-md shadow-sm border-zinc-300 focus:border-zinc-300 focus:ring focus:ring-zinc-200 focus:ring-opacity-50 text-sm">
                    </div>
                    {{-- informacion --}}
