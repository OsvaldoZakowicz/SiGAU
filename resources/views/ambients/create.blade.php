...
<div class="w-full h-max flex flex-col p-1">
    ...
    <div class="bg-zinc-300">
        ...
        <div class="p-1 flex justify-center items-center bg-zinc-400">
            <h3 class="text-sm font-bold text-zinc-800 uppercase inline-block">ambiente: crear un ambiente</h3>
        </div>
    </div>
    ...
    <div class="my-2 mx-auto w-full border bg-white border-zinc-200">
        {{-- formulario --}}
        <form action="{{ route('ambients.store') }}" method="POST">
            @csrf
                        ...
                        <div class="w-full mt-2 p-2">
                         ...
                         <div class="w-1/2 mt-2 p-2">
                            ...
                            @error('quantity')
                                <span class="text-xs text-red-600">{{ $message }}</span>
                            @enderror
                            <input type="number" name="quantity" id="quantity" min="1" max="10" value="{{old('quantity')}}" class="my-1 p-1 w-full rounded-md shadow-sm border-zinc-300 focus:border-zinc-300 focus:ring focus:ring-zinc-200 focus:ring-opacity-50 text-sm">
                        </div>
                    </div>
                    {{-- <div class="flex w-full">
                        <div class="w-1/2 mt-2 p-2">
